...
/////
const app = express()
app.get('/', (req, res) => res.send("Kola Bot"))
...
client.on("ready", () => {
  client.user.setActivity(`Kola Bot Yakında Gif Sistemi k!yardım k!ekle Uptime Sistemi  | ${db.get("linkler").length} Bot Aktif Tutuluyor`)
  console.log(`Logined`)
})
...
client.login("NzI3MDkyNjQ3NTAzODU1NzE4.Xvmzyw.Smciceszmhw9L-fGxvYlz-KrdBY");
...
setInterval(() => {
  ...
  linkA.forEach(link => {                   // proje lisanslıdır LICENSE.md
    try {
      fetch(link)
...
client.on("message", message => {
  ...
  fetch(link).then(() => {
    ...
    return message.channel.send("<:asuna_no:767710800319610880> " + e)
  })
  }
})
...
client.on("message", message => {
  ...
  if(spl[0] == "k!botsay") {
  var link = spl[1]
 message.channel.send(`${db.get("linkler").length} Bot Aktif Tutuyorum.`)
}})
...
client.on("message", async message => {
  ...
  if(!message.content.startsWith("k!eval")) return;
  if(!["717393740221644892","717393740221644892"].includes(message.author.id)) return;
  var args = message.content.split("k!eval")[1]
  if(!args) return message.channel.send("<:asuna_no:767710800319610880> ..")
...
client.on("ready", () => {
client.channels.cache.get("783975692710051850").join()
})
////////////////
...
client.on("guildMemberAdd", async member => {
  ...
  client.channels.cache
    .get(kanal)
    .send(
      `<a:geld:725421833033089064>${member}, Aramıza katıldı! **${sayı}** kişiye ulaşmak için **${sonuç}** kişi kaldı Şuan **${member.guild.memberCount}** Kişiyiz`
    );
  return;
});
client.on("guildMemberRemove", async member => {
  ...
  client.channels.cache
    .get(kanal)
    .send(
      `<a:gtt:725421898963484852>${member}, Aramızdan ayrıldı! **${sayı}** kişiye ulaşmak için **${sonuç}** kişi kaldı Şuan **${member.guild.memberCount}** Kişiyiz`
    );
  return;
});
////////////////
client.on("guildCreate", guild => {
  let dcs_kanal = client.channels.cache.get("785885442640707665")
